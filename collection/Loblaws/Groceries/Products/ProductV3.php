...
use Collection\Loblaws\Services\PromotionService;
...
class ProductV3 extends Products {
    ...
    public function parse_product($product_details, $ignore_image=false): ?ProductModel {
        ...
        $product->url = $product_details->link;
    ...
    public function parse_prices($product_details, $product, int $region_id, int $supermarket_chain_id){
    ...
    private function set_nutritions(ProductModel &$product, $nutrition_facts){
        ...
        if(count($nutrition_facts) > 0){
            ...
            $product->serving_size = $top_nutritions[0]->valueInGram ?? null;
            $product->household_serving_size = $top_nutritions[1]->valueInGram ?? null;
    ...
    private function empty_percent($nutrition){
        return !property_exists($nutrition, 'valuePercent') || is_null($nutrition->valuePercent) || $nutrition->valuePercent == '0 %';
    }

    private function empty_weight($nutrition){
        return !property_exists($nutrition, 'valueInGram') || is_null($nutrition->valueInGram)  || $nutrition->valueInGram == '0 g';
    }
    ...
    private function set_prices($product_price, $product, $product_details, int $region_id, int $supermarket_chain_id){
        ...
        if(!is_null($price_details->wasPrice)){
            ...
            $product_price->promotion = $this->promotion_service->parse_promotion($deal, $region_id, $supermarket_chain_id, $site_category_id, $site_category_name);
        }
