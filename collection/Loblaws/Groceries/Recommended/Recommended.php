...
namespace Collection\Loblaws\Groceries\Recommended;
...
class Recommended extends Loblaws {
    ...
    public function create_recommended(){
        ...
        while(true){
            ...
            $products_without_recommended = $this->product_model->select(['id','site_product_id','name'])->where(['company_id' => $this->company_id, 'recommended_searched' => null])->order_by('id','DESC')->get();
        ...
        if(!is_null($recommended_data)){
            if(property_exists($recommended_data, 'relatedProducts')){
                ...
                foreach($recommended_products as $product_data){
                    ...
                    $product_results = $this->product_model->where(['company_id' => $this->company_id, 'site_product_id' => $product_code])->first();
                    ...
                    if(is_null($product_results)){
                        ...
                        ->where(['child_categories.company_id' => $this->company_id])
