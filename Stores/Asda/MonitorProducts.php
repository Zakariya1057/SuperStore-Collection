...
class MonitorProducts extends Asda {
    ...
    function __construct(Config $config, Logger $logger, Database $database, Remember $remember=null, Notification $notification){
        ...
        $this->review = new Reviews($config, $logger, $database);
        $this->recommended = new Recommended($config, $logger, $database);
        $this->user_info = new UserModel($this->database);
        $this->product_promotions = new Promotions($config, $logger, $database);
    }
            ...
            if(!key_exists($user_id, $unique_users)){
                ...
                if(!is_null($user)){
                    ...
                    if($user->send_notifications){
                        ...
                        $this->logger->debug('User Found. Notifications Disabled');
                    }
